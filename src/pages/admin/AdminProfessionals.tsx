...
export default function AdminProfessionals() {
  ...
  const updateEditingBlock = (i: number, patch: Partial<WorkingHourBlock>) => {
    if (!editing) return;
    const wh = editing.workingHours.map((b, idx) =>
      idx === i ? { ...b, ...patch } : b,
    );
    ...
    try {
      const payload: any = {
        ...
        specialties: editing.specialties
          ...
          .map((s) => s.trim())
          .filter(Boolean),
        workingHours: editing.workingHours, // Envia os horários junto
      };

      if (!payload.name) return alert("O nome é obrigatório.");
      if (payload.specialties.length === 0)
        return alert("Informe ao menos uma especialidade.");

      if (editing.id) {
        await updateProfessional(editing.id, payload);
        // Se houver horários, atualiza-os separadamente se o seu backend exigir
      ...
      alert(e.response?.data?.message || "Erro ao salvar profissional");
    }
  };
  ...
  const saveHours = async () => {
    ...
    try {
      await updateProfessionalWorkingHours(
        hoursTarget.id,
  ...
  const updateTargetBlock = (i: number, patch: Partial<WorkingHourBlock>) => {
    if (!hoursTarget) return;
    const wh = hoursTarget.workingHours.map((b, idx) =>
      idx === i ? { ...b, ...patch } : b,
    );
    ...
    <div className="space-y-6">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 w-full sm:w-auto">
                      <input
                        ...
                        value={b.startTime}
                        onChange={(e) =>
                          updateEditingBlock(i, { startTime: e.target.value })
                        }
                      />
                      ...
                      <input
                        ...
                        value={b.endTime}
                        onChange={(e) =>
                          updateEditingBlock(i, { endTime: e.target.value })
                        }
                      />
                      <button
      ...
      {hoursTarget && (
        <Modal
          ...
          title={`Horários — ${hoursTarget.name}`}
        >
              ...
              {hoursTarget.workingHours.map((b, i) => (
                ...
                >
                  ...
                  <input
                    ...
                    value={b.startTime}
                    onChange={(e) =>
                      updateTargetBlock(i, { startTime: e.target.value })
                    }
                  />
                  <input
                    type="time"
                    ...
                    value={b.endTime}
                    onChange={(e) =>
                      updateTargetBlock(i, { endTime: e.target.value })
                    }
                  />
                  <button
                    onClick={() => {
                      const wh = hoursTarget.workingHours.filter(
                        (_, idx) => idx !== i,
                      );
                      setHoursTarget({ ...hoursTarget, workingHours: wh });
                    }}
...
function Modal({
  ...
  return (
    ...
    >
      <div
        className="bg-card rounded-2xl p-6 w-full max-w-lg shadow-2xl border border-border animate-in fade-in zoom-in duration-200"
