    ...
    <div className="flex flex-col gap-6 animate-fade-in">
      <div className="bg-muted rounded-xl p-4 space-y-2 text-sm">
        ...
        {state.form.items.map((it) => (
          <div key={it.procedure.id} className="border-b border-border/50 last:border-0 pb-2 last:pb-0">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => dispatch({ type: 'SET_PHONE', payload: formatPhone(e.target.value) })}
            className="w-full px-4 py-2.5 rounded-lg border border-input bg-background text-sm focus:outline-none focus:ring-2 ring-gold"
        ...
        <div>
          ...
          <input
            ...
            maxLength={150}
          />
          <p className="text-xs text-muted-foreground mt-1">Usado para enviar a confirmação do agendamento.</p>
        </div>
        <div>
          <label className="text-sm font-medium mb-1 block">Observações</label>
          <textarea
            value={state.form.observations}
            onChange={(e) => dispatch({ type: 'SET_OBSERVATIONS', payload: e.target.value })}
